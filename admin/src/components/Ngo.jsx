...
import { truncate } from "../store/store"
...
const Ngo = ({ ngo }) => {
  const approveNGO = async (ngoID, ngoEmail) => {
    try {
      ...
      const data = { id: ngoID, email: ngoEmail }
            ...
            <td className="text-sm text-white font-semibold px-6 py-4 whitespace-nowrap border-2 border-green-200">
              {
                ngo.isVerified === "true" ? (
                  ...
                  <div className="flex justify-between items-center">
                    <p onClick={() => approveNGO(ngo.id, ngo.email)} className="flex justify-center items-center bg-green-500 h-10 w-10 rounded-full cursor-pointer">Yes</p>
                    <p onClick={() => rejectNGO(ngo.id, ngo.email)} className="flex justify-center items-center bg-red-600 h-10 w-10 rounded-full cursor-pointer">No</p>
                  </div>
                )
              }
            </td>
