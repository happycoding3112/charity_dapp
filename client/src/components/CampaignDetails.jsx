			...
			<div className='flex flex-col justify-center p-5 bg-gray-800 rounded-lg md:w-10/12'>
				...
				<div className='flex justify-center items-center mt-6 mb-2 space-x-2'>
					{!user && campaign?.status == 0 && !expired && (
						<>
							<button
								className='leading-tight shadow-md uppercase font-semibold text-sm inline-block text-white bg-green-500 hover:bg-green-600 rounded-md px-6 py-2.5'
								onClick={() =>
									setGlobalState('fundModal', 'scale-100')
								}
							>
								donate
							</button>
							<Link to={'/user/viewPdf/' + campaign?.proofURL}>
								<button className='leading-tight shadow-md uppercase font-semibold text-sm inline-block text-white bg-blue-500 hover:bg-blue-600 rounded-md px-6 py-2.5'>
									View proof
								</button>
							</Link>
						</>
					...
					{user &&
						campaign?.owner == connectedAccount &&
						(campaign?.status != 3 ? (
							campaign?.status == 1 ? (
